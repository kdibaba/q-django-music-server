...
def music(request):
    ...
    if request.user.is_superuser or request.user.is_staff:
        home_data['album_art'] = len(Music_Album.objects.filter(album_art=True))
        home_data['artists_count'] = Music_Artist.objects.count()
        home_data['albums_count'] = Music_Album.objects.count()
        home_data['missing_album_art'] = int(home_data['albums_count']) - home_data['album_art']
        home_data['songs_count'] = Music_Song.objects.count()
    else:
        home_data['album_art'] = len(Music_Album.objects.filter(access="public").filter(album_art=True))
        home_data['artists_count'] = len(Music_Album.objects.filter(access="public").distinct("album_artist"))
        home_data['albums_count'] = Music_Album.objects.filter(access="public").count()
        home_data['missing_album_art'] = int(home_data['albums_count']) - home_data['album_art']
        home_data['songs_count'] = 0
    ...
    if request.is_ajax():
        ...
        return HttpResponse(simplejson.dumps(home_data), mimetype)
    ...
    if request.method == 'POST':
        ...
        if 'filter_nzbs' in request.POST:
            if request.POST['nzbs']:
                ...
                problems.append('You forgot to give me the location of the nzbs!')
        ...
        if str(items).count('-') > 1:
            ...
            for items2 in albums:
                if true_album in str(items2) and str(items) != str(items2):
                    ...
                    try:
                        os.rename(directory + str(items2), directory + '0/' + str(items2))
                    except:
                        print items, 'failed to move to 0\n'
        ...
        if str(items).count('-') > 1:
            ...
            for items2 in albums:
                if true_album_2 in str(items2) and str(items) != str(items2) and 'AC-DC' not in true_album_2:
                    ...
                    try:
                        os.rename(directory + str(items2), directory + '1/' + str(items2))
                    except:
                        print items, 'failed to move to 1\n'
...
def catalog_drive_music(catalog_type):
    if catalog_type == 'rebuild':
    ...
    for loop in loops:
        ...
        for drive in DRIVES.keys():
            for letter in DRIVES[drive]:
                ...
                # To avoid reprocessing files check the folders file list to the database file list.
                # Then process only the new folders.
                # Also delete albums that may have been removed from the file system.
                for current_album in current_albums_in_letter:
                ...
                # print 'Successfully deleted - %d - albums since they no longer exist in
                # the file system.\n' % album_deleted
                ...
                for file_object in files:
                    ...
                    try:
                        songs = os.listdir(directory + file_object)
                    except:
                    ...
                    for song in songs:
                        if os.path.isdir(directory + file_object + '/' + song):
                            ...
                            if genres:
                                ...
                                if genre:
                                    ...
                                    if album_genres and song_genre in album_genres.keys():
                                        ...
                                        album_genres[song_genre] = 0

                            # bitrate
                            song_bitrate = ''
                            song_bitrate = property.info.bitrate
                            if song_bitrate:
                                album_bitrate += int(song_bitrate)
                            ...
                            file_size = ''
                            try:
                                property = FLAC(directory + file_object + '/' + song)
                            except:
                                continue
                            song_length = property.info.length
                            album_length += song_length
                            ...
                            # Song genre
                            get_flac_genre = property.get('genre', None)
                            if get_flac_genre:
                                get_genre = Music_Genre.objects.filter(genre=get_flac_genre[0])
                                if not get_genre:
                                    song_genre = Music_Genre.objects.create(genre=get_flac_genre[0])
                                else:
                                    song_genre = get_genre[0]
                                if album_genres and song_genre in album_genres.keys():
                                    album_genres[song_genre] += 1
                            ...
                            id3_artist = property.get('artist', None)
                            if id3_artist:
                                get_song_artist = Music_Artist.objects.filter(artist=id3_artist[0].replace('-', '.'))
                                if not get_song_artist:
                                    song_artist = Music_Artist.objects.create(artist=id3_artist[0].replace('-', '.'))
                                    song_artist.save()
                                else:
                                    song_artist = get_song_artist[0]
                                    song_artist.save()
                            ...
                            get_album_name = property.get('album', None)
                            if get_album_name:
                                album_name = get_album_name[0]
                                if album_names and album_name in album_names.keys():
                                    album_names[album_name] += 1
                                else:
                                    album_names[album_name] = 0
                            ...
                            if not album_year:
                                get_album_year = property.get('date', None)
                                if get_album_year:
                                    album_year = get_album_year[0]
                            ...
                            get_title = property.get('title', None)
                            if not get_title:
                                title = safe_song
                            else:
                                title = get_title[0]
                            ...
                            try:
                                ...
                                shutil.move(directory + file_object + '/' + song, drive + 'DELETED/' + song + str(
                    ...
                    if not music_file_exists:
                        ...
                        if album_artists and album_artists[max(album_artists)] == song_count - 1:
                            ...
                            # print 'Going with VA since keys = ',  album_artists.keys().__len__(), '
                            # and count / 2 = ', int(song_count/2)
                        ...
                        if album_year:
                            ...
                            if string_album_year:
                                album.year = int(string_album_year[0] + string_album_year[
                                                 1] + string_album_year[2] + string_album_year[3])
                        ...
                        final_folder_name = final_artist_name + '-' + str(album.album) + '-' + album_type
                        if album.year != 0:
                            final_folder_name += '-' + str(album.year)
...
def get_album_art(query, path):
    BASE_URL = 'https://ajax.googleapis.com/ajax/services/search/images?v=1.0&q=' + \
        file_string_rename(query) + '&start=%d'
...
def album(request, album_id):
    ...
    songs = Music_Song.objects.filter(album=original_album).order_by('filename')
    all_album = []
    album = {}
    ...
    for song in songs:
        ...
        song_dict['length'] = song.get_song_length()
...
def albums(request, letter):
    ...
    if request.user.is_superuser or request.user.is_staff:
        if letter == 'all':
            all_albums = Music_Album.objects.all().values('album', 'album_size', 'album_genre__genre', 'album_genre_id', 'album_artist__artist', 'album_artist_id',
                                                                    'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
        elif letter == '0':
            all_albums = Music_Album.objects.filter(album__regex=r'\d').values('album', 'album_size', 'album_genre__genre', 'album_genre_id', 'album_artist__artist', 'album_artist_id',
                                                                    'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
        else:
            letter_albums = Music_Album.objects.filter(
                album__istartswith=letter).order_by('album').values('album', 'album_size', 'album_genre__genre', 'album_genre_id', 'album_artist__artist', 'album_artist_id',
                                                                    'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
            albums_with_the = Music_Album.objects.filter(
                album__istartswith='THE ' + letter).order_by('album').values('album', 'album_size', 'album_genre__genre', 'album_genre_id', 'album_artist__artist', 
                                                                            'album_artist_id', 'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
    else:
        if letter == 'all':
            all_albums = Music_Album.objects.filter(
                access="public").values('album', 'album_size', 'album_genre__genre', 'album_genre_id', 'album_artist__artist', 'album_artist_id',
                                                                    'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
        elif letter == '0':
            all_albums = Music_Album.objects.filter(
                access="public").filter(album__regex=r'\d').values('album', 'album_size', 'album_genre__genre', 'album_genre_id', 'album_artist__artist', 'album_artist_id',
                                                                    'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
        else:
            letter_albums = Music_Album.objects.filter(
                access="public").filter(album__istartswith=letter).order_by('album').values('album', 'album_size', 'album_genre__genre',
                                                                                            'album_genre_id', 'album_artist__artist', 'album_artist_id', 'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
            albums_with_the = Music_Album.objects.filter(
                access="public").filter(album__istartswith='THE ' + letter).order_by('album').values('album', 'album_size', 'album_genre__genre',
                                                                                                     'album_genre_id', 'album_artist__artist', 'album_artist_id', 'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')

    if not all_albums:
        dictionary_albums = list(letter_albums) + list(albums_with_the)
    else:
        dictionary_albums = all_albums
...
def albums_by_year(request, year_id):
    if request.user.is_superuser or request.user.is_staff:
        all_albums = Music_Album.objects.filter(
            year=year_id).order_by('album').values('album', 'album_size', 'album_genre__genre',
                                                   'album_genre_id', 'album_artist__artist', 'album_artist_id', 'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
    else:
        all_albums = Music_Album.objects.filter(
            access="public").filter(year=year_id).order_by('album').values('album', 'album_size', 'album_genre__genre',
                                                                           'album_genre_id', 'album_artist__artist', 'album_artist_id', 'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
...
def albums_by_genre(request, genre_id):
    if request.user.is_superuser or request.user.is_staff:
        all_albums = Music_Album.objects.filter(
            album_genre=genre_id).order_by('album').values('album', 'album_size', 'album_genre__genre',
                                                           'album_genre_id', 'album_artist__artist', 'album_artist_id', 'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
    else:
        all_albums = Music_Album.objects.filter(
            access="public").filter(album_genre=genre_id).order_by('album').values('album', 'album_size', 'album_genre__genre',
                                                                                   'album_genre_id', 'album_artist__artist', 'album_artist_id', 'drive', 'song_count', 'letter', 'year', 'folder', 'id', 'album_art')
...
def artists(request, letter):
    ...
    if letter == 'all':
        ...
        get_artists = Music_Artist.objects.filter(artist__regex=r'\d').order_by('artist')
    else:
        get_artists = Music_Artist.objects.filter(artist__istartswith=letter).order_by('artist')
    ...
    for get_artist in get_artists:
        ...
        if request.user.is_superuser or request.user.is_staff:
            ...
            get_albums = Music_Album.objects.filter(access="public").filter(
                album_artist=get_artist.id).order_by('id')
        if get_albums:
    ...
    if request.user.username == 'kdibaba':
        ...
        for key in audio.keys():
            if 'POPM' in key:
                ...
                audio.save()
        if not key_found:
            audio.add(mutagen.id3.POPM(email=u'qrating', rating=SET_RATING[str(rating)]))
            audio.save()
        song.rating = rating
        song.save()
        message = 1
...
def copy_favs(request):
    ...
    for songs in five_star_songs:
        try:
            os.makedirs(DEFAULT_DIRECTORY + '/' + 'FAVS' + '/' +
                        songs.album.album_artist.artist.upper().replace('.', ' '))
        except:
            pass
        try:
...
#
    ...
    if not existing_playlists:
        for song in song_ids:
            Music_Playlist.objects.create(name=playlist_name, song=Music_Song.objects.get(
                id=int(song)), visible_to_others=visible, user=request.user, note=note)
