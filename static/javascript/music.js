...
function show_artists(jsonArtists) {
	...
	$.each(artists, function(i,item){
		artist_count+=1;
		$('#artists').append('<li onclick="get_albums_by_artist('+item.pk+')">' + item.fields.artist + ' </li>');
	})
	$('#artists').append('</ul>');
...
function show_albums(jsonAlbums) {
	...
	$.each(albums, function(i,item){
		...
		if (item.fields.album_art){$('.album_'+item.pk).append( '<div id="img_div"><img src="/static/music/A/' + item.fields.folder + '/Folder.jpg' + '" /></div>' )}
...
function show_album(jsonAlbum) {
	...
	$.each(CURRENT_ALBUM, function(i,item){
		if (item.fields.album_art){$('.current_album').append( '<div id="curr_album_img_div"><img src="/static/music/A/' + item.fields.folder + '/Folder.jpg' + '" /></div>' )}
	...
	$.each(album, function(i,item){
		if (item.fields.type == 'mp3'){
			$('#song_table').append('<tr class="songs" id="song_'+item.pk+'" ><td ondblclick="play_song(\''+item.pk+'\', \'' + item.fields.type + '\', \'' + item.fields.path + '\', \'' + item.fields.filename + '\')">' + item.fields.title + '</td>' +
